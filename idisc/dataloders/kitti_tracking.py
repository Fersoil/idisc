...
class KITTITrackingDataset(BaseDataset):
    CAM_INTRINSIC = {
    ...
    def load_dataset(self):
        ...
        for img_name in image_files:
            img_info = {
                "image_filename": os.path.join(self.base_path, img_name),
                "camera_intrinsics": self.CAM_INTRINSIC[self.intrinsic_key][:, :3],
            }
            self.dataset.append(img_info)

        print(
            f"Loaded {len(self.dataset)} images from folder {self.base_path}. "
            f"{self.invalid_depth_num} invalid depth pairs skipped (0 for this folder mode)."
        )
    ...
    def __getitem__(self, idx):
        ...
        return {"image": image, "gt":  torch.zeros(1), "mask":  torch.zeros(1)}
