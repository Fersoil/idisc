...
def main():
    ...
    if args.sam_checkpoint is None and args.sam3_cache_dir is None:
        parser.error("Either --sam-checkpoint or --sam3-cache-dir is required")
    use_online_sam = args.sam_checkpoint is not None
    ...
    if use_online_sam:
        print(f"Prompt: {args.prompt_mode}", flush=True)
    else:
        print(f"Cache: {args.sam3_cache_dir}", flush=True)
    if device.type == "cuda":
    ...
    # Datasets
    cache_dir = args.sam3_cache_dir if not use_online_sam else None
    ...
    if use_online_sam:
        ...
        sam_model = build_sam3_image_model(
            device=str(device), checkpoint_path=args.sam_checkpoint, load_from_HF=False)
        sam_model.eval()
    ...
    scheduler = OneCycleLR(
        optimizer, max_lr=args.lr, total_steps=args.n_iters,
    ...
    print(f"\nStart fine-tuning for {args.n_iters} iterations (lr={args.lr})...", flush=True)
    start = time()
    step = 0
    model.train()
        ...
        for batch in train_loader:
            ...
            for idx in range(data.shape[0]):
                ...
                if use_online_sam:
                    raw_img = denormalize_imagenet(data[idx])
                    with torch.no_grad():
                        iq = extract_online_queries(sam_proc, raw_img, args.prompt_mode)
                    if iq is not None:
                        iq = iq.to(device)
                elif sam3_q is not None:
                    q = sam3_q[idx]
            ...
            if step % 100 == 0:
                ...
                eta = int(elapsed * (args.n_iters - step) / max(1, step))
                lr_now = scheduler.get_last_lr()[0]
                print(
                    f"  Step {step}/{args.n_iters} | loss={total_loss:.5f} | "
                    f"lr={lr_now:.2e} | [{format_seconds(elapsed)}<{format_seconds(eta)}]",
                    flush=True,
                )

            if step % args.val_interval == 0 or step == args.n_iters:
                ...
                with torch.no_grad():
                    metrics = validate_model(model, valid_loader, context, device,
                                             ...
                                             prompt_mode=args.prompt_mode if use_online_sam else None)
                model.train()
                for param in model.parameters():
                    param.requires_grad = False
                for param in model.sam3_proj.parameters():
                ...
                if abs_rel < best_abs_rel:
                    ...
                    ckpt_path = os.path.join(args.output_dir, "best_sam_finetuned.pt")
    ...
    # Final save
    final_path = os.path.join(args.output_dir, "final_sam_finetuned.pt")
